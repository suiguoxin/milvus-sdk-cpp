...
main(int argc, char* argv[]) {
    ...
    std::string filter_filename = "/embeddings/or_filter_query.tsv";
    ...
    std::vector<int> ingre_num, instruct_step;
    ...
    for (; getline(filter_in, line);) {
        ...
        line.erase(0, line.find("\t") + 1);
        instruct_step.push_back(std::stoi(line.substr(0, line.find("\n"))));
        num++;
    }
